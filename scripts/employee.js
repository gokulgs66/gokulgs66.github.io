...
let employeeHtml = document.getElementsByClassName("empbox")[0];
let empListHtml = '';

for(let i=0; i<data.length;i++){
    employeeHtml.getElementsByClassName("fname")[0].textContent = data[i].Name;
    employeeHtml.getElementsByClassName("name2")[0].textContent = data[i].EmployeeID;
    employeeHtml.getElementsByTagName("button")[0].setAttribute('onclick','view('+i+')')
    empListHtml += '<div class="empbox">' + employeeHtml.innerHTML + '</div>';
}

document.getElementsByClassName("empbox")[0].setAttribute('style', 'display:none;')
document.getElementById("demo").innerHTML = empListHtml;


function view(i) {
  localStorage.setItem('emp', JSON.stringify(data[i]));
